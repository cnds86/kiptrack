...
const App: React.FC = () => {
  ...
  // 2. Cloud Data Listener (Only if logged in)
  useEffect(() => {
  ...
  // 3. LocalStorage Saver (Only if NOT logged in)
  useEffect(() => {
    if (currentUser) return; // Don't save to LS if using Cloud
    const data: AppData = {
      accounts,
      transactions,
      incomeCategories,
  ...
  }, [accounts, transactions, incomeCategories, expenseCategories, currencies, goals, recurringTransactions, notifications, language, currentUser]);

  // 4. LocalStorage Loader Helper
  ...
  useEffect(() => {
    ...
    saveUserDataToCloud(currentUser.uid, data);
  }, [accounts, transactions, incomeCategories, expenseCategories, currencies, goals, recurringTransactions, notifications, currentUser]);
